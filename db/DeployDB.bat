...
SET DatabaseName=AMERIA_SME_SCORING_2
...
for %%G in (Dummy/Tables/*.sql) do %SQLEXEC% -d %DB_Name% -i "Dummy/Tables/%%G"
...
for %%G in (Dummy/CLR/*.sql) do %SQLEXEC% -d %DB_Name% -i "Dummy/CLR/%%G"
