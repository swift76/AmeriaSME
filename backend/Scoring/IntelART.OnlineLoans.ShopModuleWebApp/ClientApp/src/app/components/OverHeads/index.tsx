...
import { IOverheadsData, ISelectDataReceive } from 'app/store/reducers/common-models';
...
const OverHeads: React.FC<IOverHeadsProps> = props => {
    ...
    const { loading, values, touched, errors, handleChange, setFieldValue, setError } = props;
    ...
    useEffect(() => {
        for (const iterator of data) {
            if (iterator.PRODUCTS.length < industryProducts.data.length && iterator.CODE !== '') {
    ...
    const getIsValid = (name: string) => getIn(touched, name) && !getIn(errors, name);

    const getIsInValid = (name: string) => getIn(touched, name) && !!getIn(errors, name);
