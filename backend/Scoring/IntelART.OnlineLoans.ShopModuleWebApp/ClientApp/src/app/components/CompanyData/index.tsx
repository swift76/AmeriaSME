...
const AppCompanyData: React.FC<IAppCompanyDataProps> = props => {
  ...
  const setArrayFieldComment = (
    index: number,
    row: IAppCompanyData,
    action: any
  ) => (nCom: string) => action(index, { ...row, ...{ COMMENT: nCom } })

  const setFieldComment = (name: string, action: any) => (nCom: string) =>
    action(name, nCom)
              ...
              render={arrayHelpers => (
                <>
                  {data.length ? (
                        ...
                        >
                          <td className="name-col">
                            {editable ? (
                              <Form.Group controlId={codeName}>
                                <Form.Control
                                  ...
                                  onChange={e => {
                                    handleChange(e)
                                    sync &&
                                      handleChangeSyncFields(e, sync, index)
                                  }}
                                  className="control-200"
                                  isValid={
                                    getTouched(codeName) && !getError(codeName)
                                  }
                                  isInvalid={
                                    getTouched(codeName) && !!getError(codeName)
                                  }
                              ...
                              row.NAME ||
                              Utils.getNameByCode(row.CODE, industryTypes.data)
                            )}
                          </td>
                          ...
                          <td>
                            <Form.Group controlId={name}>
                              <NumberFormat
                                ...
                                onFocus={() => {
                                  data[index].AMOUNT === 0 &&
                                    setFieldValue(amountName, '')
                                }}
                                onValueChange={val => {
                                  setFieldValue(amountName, val.value)
                                }}
                    ...
                    <>
                      <tr>
                        <td colSpan={4}>{`Չկա ավելացված ${title}ի տեսակ`}</td>
                      </tr>
              ...
              return (
                ...
                >
                  ...
                  <td colSpan={2}>
                    ...
                    >
                      {!!commentVal ? commentVal : `Ավելացնել մեկնաբանություն`}
                    </Button>
                  </td>
                </tr>
              )
