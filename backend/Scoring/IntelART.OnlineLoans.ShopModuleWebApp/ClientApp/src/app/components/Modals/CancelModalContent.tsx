...
import CancelModalContainer, {
  ICancelModalProps,
} from '@componentContainers/CancelModalContainer'
...
const Component: React.FC<ICancelModalProps> = props => {
  ...
  const validationSchema = yup.object({
    CANCELLATION_REASON_CODE: yup
      .string()
      .required('Խնդրում ենք ընտրել մերժման պատճառը'),
  })
                ...
                <Form.Group controlId="formCancelRadios">
                  {cancelReasons.data.map(reason => (
                    <Form.Check
                      ...
                      checked={values.CANCELLATION_REASON_CODE === reason.CODE}
                      value={reason.CODE}
