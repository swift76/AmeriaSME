...
import { loanSpecApplicationActions as actions } from 'app/store/reducers/root-actions';
...
export const saveLoanSpecApplication = (id: string, data: ILoanSpecApplicationData) => async (
    dispatch: Dispatch<AnyAction>
) => {
    dispatch(actions.postLoanSpecApplicationRequrest());
    try {
        const response = await axios.post(`/Applications/${id}/LoanSpecialist`, data);
