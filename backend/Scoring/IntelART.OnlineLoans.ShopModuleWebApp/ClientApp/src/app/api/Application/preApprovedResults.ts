...
import { preapprovedResultsActions as actions } from 'app/store/reducers/root-actions';
import axios from 'app/api';
import { toast } from 'react-toastify';

export const getPreApprovedResults = (id: string) => async (dispatch: Dispatch<AnyAction>) => {
...
export const savePreApprovedResult = (id: string, data: IPreApprovedResultsPostData) => async (
    dispatch: Dispatch<AnyAction>
) => {
    dispatch(actions.postPreApprovedResultsRequest());
    try {
        const response = await axios.post(`/Applications/${id}/PreapprovedResult`, data);
