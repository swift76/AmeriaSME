...
import { resetMainApplication } from 'app/store/reducers/mainApplication/actions';
import { resetScoringResults } from 'app/store/reducers/scoringResults/actions';

export default (Page: React.FC<IMainAppProps>) => {
    const MainApplicationContainer = (props: IMainAppProps) => <Page {...props} />;

    return withRouter(connect(mapStateToProps, mapDispatchToProps)(MainApplicationContainer));
};
...
const mapDispatchToProps = (dispatch: AppTypes.DispatchActions) => {
    return {
        ...
        saveMainApplication: (id: string, data: IPostMainApplicationData) =>
            dispatch(saveMainApplication(id, data)),
