...
function validateForm(container) {	
	var validateTextInputs = function(){
		...
		for(var i = 0; i < textInputs.length; i++){
			...
			if(vtype == "phone" && !isValidPhone(input.value)){
				showErrorOnField(input,PHONE_FORMAT_ERROR);
				result = false;
				continue;
			}
...
function getObjectByCriteria(objArr, key, value){
...
function formatPhoneNumber(num){
	num = num.splice(num.length-2, 0, "-")
	num = num.splice(num.length-5, 0, "-")
	num = num.splice(num.length-8, 0, "-")
	return "374 "+num;
}
