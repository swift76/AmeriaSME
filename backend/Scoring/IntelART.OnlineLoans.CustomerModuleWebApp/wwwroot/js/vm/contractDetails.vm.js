define(['knockout',
    ...
    function (ko, $, ActiveCreditCardList, CreditCardTypeList, BankBranchList, LoanDocsModel, CompanyInfoModel, Diff, helpers, LookupDirectory, NotificationDialog) {

    ContractDetails = function (id, preApprovalStage, navigationHelper, isEditable) {
        var self = this;
        ...
        this.activeCardListVisible = ko.computed(function () {
            return (self.activeCreditCards() && self.activeCreditCards().items() && self.activeCreditCards().items().length > 0);
        });
        ...
        this.isSubmitButtonDisabled = ko.pureComputed(function () {
            return !self.isEditable() ||
                    ...
                    (self.guaranteeAgreementLinkVisible() && (self.guaranteeTermsCheckbox() === false || !self.guaranteeAgreementText()));
        });

        this.isApplicationEditable = ko.computed(function () {
            return (self.state() !== "COMPLETED" && self.state() !== "PRINT_READY" && self.state() !== "AGREED");
        });
        ...
        this.arbitrageAgreementLink = LoanDocsModel.getArbitrageAgreementUrl(self.id);
        ...
        this.guaranteeAgreementLinkVisible = ko.computed(function () {
            CompanyInfoModel.checkCompanyLLC(self.id).then(function (isLLC) {
                if (isLLC) {
                    ...
                    self.guaranteeAgreementLink = LoanDocsModel.getGuaranteeAgreementUrl(self.id);
                    return isLLC;
                }
            });
                ...
                $.get({
                    ...
                    success: function (data) {
                        if (data) {
                            ...
                            self.guaranteeSignatureText(data.GUARANTEE_SIGNATURE_TEXT_TO_BE_ENTERED);
                            self.state(data.STATUS_STATE);

                            if (self.state() === "COMPLETED" || self.state() === "PRINT_READY" || self.state() === "AGREED") {
                            ...
                            if (data.CARD_DELIVERY_ADDRESS) {
                                self.cardDeliveryAddress = ko.observable(data.CARD_DELIVERY_ADDRESS);
                            }
        ...
        this.action = function (name, callback) {
            var self = this;
            if (name == 'loanContractLink' || name == 'pledgeAgreementLink' || name == 'guaranteeAgreementLink') {
        ...
        this.validate = function () {
            ...
            if (self.isCardAccount()) {
                if (!self.newCardCheckbox() && !self.activeCreditCard()) {
                    self.validationErrors.push({ propertyName: 'activeCreditCard', errorMessage: localization.errors["CARD_REQUIRED_ERROR"] });
                }

                if (self.newCardCheckbox()) {
                    if (!self.creditCardType()) {
                        self.validationErrors.push({ propertyName: 'creditCardType', errorMessage: localization.errors["CARD_TYPE_REQUIRED_ERROR"] });
                    }

                    if (self.isCardDeliveryAddressVisible() && (!self.cardDeliveryAddress() || self.cardDeliveryAddress().trim() == "")) {
                        self.validationErrors.push({ propertyName: 'cardDeliveryAddress', errorMessage: localization.errors["REQUIRED_FIELD_ERROR"] });
                    }

                    if (!self.isCardDeliveryAddressVisible() && !self.bankBranch()) {
                        self.validationErrors.push({ propertyName: 'bankBranch', errorMessage: localization.errors["BRANCH_REQUIRED_ERROR"] });
                    }
                }
            }
            ...
            if (self.guaranteeAgreementLinkVisible()) {
            	if (!self.guaranteeAgreementText()) {
            		self.validationErrors.push({ propertyName: 'guaranteeAgreementText', errorMessage: localization.errors["REQUIRED_FIELD_ERROR"] });
            	} else if (!self.checkguaranteeAgreementText(self.guaranteeAgreementText())) {
            		var diffContent = self.diffGuaranteeAgreementText(self.guaranteeAgreementText());
            		self.diffHtml(diffContent);
            		self.validationErrors.push({ propertyName: 'guaranteeAgreementText', errorMessage: localization.errors["GUARANTEEAGREEMENTTEXTERROR"] });
            	} else {
            		self.diffHtml('');
            	}
            }
        ...
        this.submitContractDetails = function (callback) {
            ...
            var dialog = new NotificationDialog({
                message: "Կատարվում են բանկային ձևակերպումները, խնդրում ենք սպասել, դա կարող է տևել մի քանի րոպե:",
                ...
                confirm: function () { navigationHelper.navigateToApplicationList(); }
            });
            dialog.show();
        ...
        this.checkguaranteeAgreementText = function (text) {
            ...
            return text.replace(regexp, '') === self.guaranteeSignatureText().replace(regexp, '');
        }
        ...
        this.diffGuaranteeAgreementText = function(text) {
			...
			var color = '', weigth = "400", span = null,
			content = $('<div></div>');
            diffArr.forEach(function(part){
                color = part.added ? '#3da08b' :
                part.removed ? '#c40000' : '#999';
