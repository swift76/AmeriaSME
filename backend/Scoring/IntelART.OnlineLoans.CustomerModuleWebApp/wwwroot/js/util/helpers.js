define(["jquery", "knockout", "./util/notificationDialog"], function (
  $,
  ko,
  NotificationDialog
) {
  return {
    ...
    confirmCancelModal: function (value, CancellationReasons, confirmAction) {
      var dialog = new NotificationDialog({
        ...
        dropdown: {
          label:
            'Խնդրում ենք ընտրել չեղարկման պատճառը <span class="mandatory">*</span>',
    ...
    cleanConfirmText: function (txt) {
      var cleanText = txt.toLowerCase().replaceAll("․", ".").replace(/\s/g, "");
      return cleanText;
    },
    actualInterestUserText:
